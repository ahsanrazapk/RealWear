  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: SafeArea(
        child: Row(
          children: [
            Sidebar(onItemSelected: (bool isParent, int parentIndex, int? childIndex) {
            ...
            Expanded(
              child: Center(
                child: Text('parentIndex: $parentIndex \n childIndex: $childIndex'),
              ),
            )
          ],
        ),
...
class Sidebar extends StatefulWidget {
  ...
  Sidebar({Key? key, required this.onItemSelected}) : super(key: key);
...
class _SidebarState extends State<Sidebar> {
  final List<TabItem> tabs = [
    TabItem(
      ...
      globalKey: GlobalKey(),
      children: [
        TabItem(title: '1', globalKey: GlobalKey(),),
        TabItem(title: '2', globalKey: GlobalKey(),),
        TabItem(title: '3', globalKey: GlobalKey(),),
      ],
    ),
  ...
  Widget _buildTile(TabItem tab, int index) {
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(color: ColorManager.subStep, borderRadius: BorderRadius.circular(16),),
      ...
      child: Column(
        children: [
          ...
          ...tab.children!.map((e) => Padding(
            padding: const EdgeInsets.all(4.0),
            child: Text(
              key: e.globalKey,
              e.title,
              textAlign: TextAlign.center,
              style: context.textTheme.displayLarge?.copyWith(color: tab.children!.indexOf(e) == _childSelectedIndex ?ColorManager.secondary:Colors.white),
            ),
          ),)
        ],
      ),
